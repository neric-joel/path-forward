...
| V2 | `442c9ce` | School matching (3 schools + cost breakdowns), semester roadmap, 8 ethical safeguards, demo mode. Hit truncation bug (22k chars > 8k max_tokens). |
...
| V3.2 | `efc4bc1` | Planned start semester in intake Step 3. PLANNED_START_RULES in prompt-base.ts. All prompts use it. |
...
- **Schools**: On-demand + contextual questions (TabQuestions). 3 school cards with cost breakdowns, foster support, housing, fit_label.
...
- CLAUDE_API_KEY server-side only (never exposed to browser)
- Validates maxTokens against allowlist (2000/2500/3000/4000)
- Client calls `/api/assess` instead of Anthropic directly

### ✅ Per-Section Prompts

- `src/lib/prompts/prompt-base.ts` — shared principles, 8 safeguards, PLANNED_START_RULES, SCORING_RULES
...
- "Try Demo" on landing page loads per-section demo data (DEMO_OVERVIEW, DEMO_FINANCIAL, etc.)
- Amber "Demo Mode" banner on dashboard
- Per-section fallback if any API call fails

### ✅ Other

- PDF export (jsPDF) — cover page + all generated sections + footer on every page
- Score delta engine (client-side, no second API call)
- Privacy banner on every page
- vercel.json with SPA rewrites + /api/ passthrough
...
- Use `senior-architect` before wiring new state across tabs
- Use `senior-prompt-engineer` exclusively when editing any file in `src/lib/prompts/`
...
```
User → Intake Form (6 steps, incl. planned_start)
         ↓
...
**Stack:** React 18 + TypeScript + Vite + Tailwind CSS + React Router v6 + Claude API via serverless proxy + jsPDF + Vercel
...
│   ├── components/
│   │   ├── intake/
│   │   │   ├── IntakeForm.tsx          # 6 steps (planned_start ✅)
│   │   │   ├── StepIndicator.tsx
│   │   │   ├── IntakeReview.tsx
│   │   │   └── fields/
│   │   │       ├── AgeStateField.tsx
│   │   │       ├── EducationGoalField.tsx
│   │   │       ├── TimelineField.tsx    # ✅ Includes planned_start picker
│   │   │       ├── DocumentsField.tsx
│   │   │       └── BenefitsField.tsx
│   │   ├── dashboard/
│   │   │   ├── DashboardView.tsx       # Tabbed layout + PDF export button
│   │   │   ├── TabBar.tsx              # 5 tabs
│   │   │   ├── SectionIntro.tsx        # Pre-generation CTA card
│   │   │   ├── TabQuestions.tsx        # Contextual questions (radio/multiselect/sensitive)
│   │   │   ├── OverviewTab.tsx
│   │   │   ├── FinancialAidTab.tsx
│   │   │   ├── SchoolsTab.tsx          # + location/priorities questions
│   │   │   ├── ActionPlanTab.tsx       # + housing/income questions (sensitive)
│   │   │   ├── RoadmapTab.tsx          # School picker + attendance questions
...
│   │   │   └── SemesterRoadmap.tsx     # Phase visual identity (🧭📖☀️🎓)
│   │   └── shared/
│   │       ├── LoadingSkeleton.tsx / ConfidenceBadge.tsx / SourceCitation.tsx
│   ├── lib/
│   │   ├── claude.ts                   # Per-section fetch functions + dual routing
│   │   ├── demo-data.ts                # Per-section demo data
│   │   ├── prompts/
│   │   │   ├── prompt-base.ts          # ✅ Includes PLANNED_START_RULES
│   │   │   ├── overview-prompt.ts
│   │   │   ├── financial-prompt.ts
│   │   │   ├── schools-prompt.ts
...
4. Overconfident Rankings → fit_label not raw scores
...
# Client-side (local dev fallback — bypasses serverless proxy)
