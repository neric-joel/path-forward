  ...
  return (
    <div ref={ref} className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="text-[15px] font-semibold text-[#1C1C1A]">{label}</span>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[15px] font-bold" style={{ color: barColor }}>{score}</span>
        </div>
      </div>
      ...
      {summary && (
        <p className="text-[13px] text-[#6B6A65] leading-relaxed">{summary}</p>
      )}
    </div>
  );
}
